...
Func CollectElixirCart($bSwitchToBB = False, $bSwitchToNV = False)
	...
	Local $bRet, $aiElixirCart, $aiCollect, $iRandomX, $iRandomY
	$iRandomX = Random(530, 540, 1)
	$iRandomY = Random(140, 150, 1)
	If Not CheckBBElixirStorageFull(False) Then
		SetLog("Found Filled Elixir Cart", $COLOR_SUCCESS)
		PureClick($iRandomX, $iRandomY) ; Click location of the elixir cart.
		...
		CloseWindow(20)
	Else
			SetLog("Elixir storage is full!", $COLOR_ERROR)
	EndIf
