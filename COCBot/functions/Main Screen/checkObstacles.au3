; #FUNCTION# ====================================================================================================================
; Name ..........: checkObstacles
; Description ...: Checks whether something is blocking the pixel for mainscreen and tries to unblock with reduced delays
; Syntax ........: checkObstacles([$bBuilderBase = Default])
; Parameters ....: $bBuilderBase - Check Builder Base context
; Return values .: Returns True when there is something blocking
; Author ........: Hungle (2014)
; Modified ......: KnowJack (2015), Sardo (08-2015), TheMaster1st(10-2015), MonkeyHunter (08-2016), MMHK (12-2016), Moebius (11-2024), [Your Name] (04-2025)
...
Func checkObstacles($bBuilderBase = Default)
    ...
    If UBound(decodeSingleCoord(FindImageInPlace2("Update", $g_sImgCOCUpdate, 220, 165 + $g_iMidOffsetY, 350, 260 + $g_iMidOffsetY, True))) > 1 Or _
            UBound(decodeSingleCoord(FindImageInPlace2("UpdateNew", $g_sImgCOCUpdate, 350, 370 + $g_iMidOffsetY, 510, 450 + $g_iMidOffsetY, True))) > 1 Then
        SetLog("Chief, we have COC Update!", $COLOR_INFO)
        ClickAway()
        If _Sleep(100) Then Return ; Reduced from $DELAYCHECKOBSTACLES1 (1000ms) to 100ms
    EndIf
...
Func _checkObstacles($bBuilderBase = False, $bRecursive = False)
    ...
    If UBound(decodeSingleCoord(FindImageInPlace2("Loading", $g_sImgLoading, 380, 570 + $g_iBottomOffsetY, 490, 620 + $g_iBottomOffsetY, False))) > 1 Then
        $Result = getOcrReloadMessage(290, 320 + $g_iMidOffsetY, "Check Obstacles OCR 'policy at super'=")
        ...
        EndIf
        $Result = getOcrReloadMessage(410, 276 + $g_iMidOffsetY, "Check Obstacles OCR 'prohibited 3rd'= ")
    ...
    Else
        ...
        If IsArray($aPixelSearchLight) And IsArray($aPixelSearchGreenContinue) Then
            ...
            If IsArray($aContinueButton) And UBound($aContinueButton, 1) = 2 Then
                ...
                If _Sleep(1500) Then Return ; Kept $DELAYTREASURY2 (1500ms) as UI transition may need it
                $g_bMinorObstacle = True
                Return False
            EndIf
        EndIf
    ...
    If UBound(decodeSingleCoord(FindImageInPlace2("Maintenance", $g_sImgMaintenance, 250, 40, 500, 100 + $g_iMidOffsetY, False))) > 1 Then
        $Result = getOcrMaintenanceTime(285, 581 + $g_iMidOffsetY, "Check Obstacles OCR Maintenance Break=")
        ...
        If $g_bNotifyTGEnable And $g_bNotifyAlertMaintenance Then NotifyPushToTelegram("Maintenance Break, waiting: " & $iMaintenanceWaitTime / 60000 & " minutes....")
    ...
    If _ColorCheck(_GetPixelColor(810, 185 + $g_iMidOffsetY, $g_bCapturePixel), Hex(0x892B1F, 6), 20) Then
        ...
        PureClick(440, 510 + $g_iMidOffsetY, 1, 120, "#0132")
    ...
    If Not $bHasTopBlackBar And IsMainGrayed() Then
        ...
        Else
            ...
            If IsMainGrayed() Then
                ...
                While 1
                    ClickAway()
                    If _Sleep(100) Then ExitLoop ; Reduced from $DELAYCHECKOBSTACLES1 (1000ms) to 100ms
    ...
    If _CheckPixel($aCancelFight, $g_bCapturePixel) Or _CheckPixel($aCancelFight2, $g_bCapturePixel) Then
        ...
        PureClickP($aCancelFight, 1, 120, "#0135")
        $g_bMinorObstacle = True
        If _Sleep(100) Then Return ; Reduced from $DELAYCHECKOBSTACLES1 (1000ms) to 100ms
        Return False
    EndIf
    ...
    If _CheckPixel($aEndFightSceneBtn, $g_bCapturePixel) Then
        ...
        PureClickP($aEndFightSceneBtn, 1, 120, "#0137")
        If _Sleep(100) Then Return ; Reduced from $DELAYCHECKOBSTACLES1 (1000ms) to 100ms
        Return True
    EndIf
    If _CheckPixel($aSurrenderButton, $g_bCapturePixel) Then
        ...
        Return True
    EndIf
    If _CheckPixel($aNoCloudsAttack, $g_bCapturePixel) Then
        ...
        If IsArray($aMessage) Then
            ...
            PureClick(67, 602 + $g_iBottomOffsetY, 1, 120, "#0138")
            If _Sleep(200) Then Return ; Reduced from $DELAYCHECKOBSTACLES2 (2000ms) to 200ms
            Return True
        EndIf
    EndIf
    If IsPostDefenseSummaryPage() Then
        ...
        If IsArray($aMessage) Then
            ...
            PureClick(62, 607 + $g_iBottomOffsetY, 1, 120, "#0138")
            If _Sleep(200) Then Return ; Reduced from $DELAYCHECKOBSTACLES2 (2000ms) to 200ms
            Return True
        EndIf
    EndIf
    ...
    If UBound($CSFoundCoords) > 1 Then
        ...
        PushMsg("CoCError")
        If _Sleep(100) Then Return ; Reduced from $DELAYCHECKOBSTACLES1 (1000ms) to 100ms
        PureClick($CSFoundCoords[0], $CSFoundCoords[1], 1, 120, "#0129")
        If _Sleep(200) Then Return ; Reduced from $DELAYCHECKOBSTACLES2 (2000ms) to 200ms
        Return checkObstacles_ReloadCoC($bRecursive)
    EndIf

    If UBound(decodeSingleCoord(FindImageInPlace2("MaintClock", $g_sImgMaintClock, 760, 160 + $g_iMidOffsetY, 860, 230 + $g_iMidOffsetY, True))) > 1 Then
        If isOnMainVillage() Then
            ...
            If _Sleep($DELAYRUNBOT3) Then Return
            Local $iWaitTime = Random(120, 240, 1)
...
Func checkObstacles_ReloadCoC($bRecursive = False)
    ...
    If _Sleep(500) Then Return ; Reduced from $DELAYCHECKOBSTACLES3 (5000ms) to 500ms
    Return True
...
Func ClashOfMagicAdvert($bDebugImageSave = $g_bDebugImageSave)
    ...
    If IsArray($aiClashOfMagicAdvert) And UBound($aiClashOfMagicAdvert, 1) = 2 Then
        ...
        If _Sleep(100) Then Return ; Reduced from 500ms to 100ms
...
Func CheckAllObstacles($bDebugImageSave = $g_bDebugImageSave, $MinType = 0, $MaxType = 7, $bRecursive = False)
    ...
    For $i = $MinType To $MaxType
        ...
        If IsArray($aiObstacleType[$i][0]) And UBound($aiObstacleType[$i][0]) = 2 Then
            ...
            If _Sleep(50) Then Return ; Reduced from 100ms to 50ms
...
Func CheckDailyRewardWindow()
    ...
    If _Sleep(50) Then Return ; Reduced from $DELAYSTARBONUS100 (assumed 100ms) to 50ms
    ...
    If $aResult <> "" And IsArray($aResult) Then
        For $t = 0 To UBound($aResult) - 1
            ...
            If IsArray($aResultArray) And $aResultArray[0] = "ClaimBtn" Then
                $sAllCoordsString = _ArrayToString($aResultArray, "|", 1)
                $aAllCoordsTemp = decodeMultipleCoords($sAllCoordsString, 50, 50)
        ...
        For $j = 0 To UBound($aAllCoords) - 1
            Click($aAllCoords[$j][0], $aAllCoords[$j][1], 1, 120, "Claim " & $j + 1)
            If _Sleep(200) Then Return ; Reduced from 2000ms to 200ms
        Next
    EndIf
    CloseWindow2()
...
Func IsMainGrayed()
    Local $offColors1[2][3] = [[0x3D5F72, 4, 0], [0x7B7B77, 9, 0]]
    Local $IsMainGrayed1 = _MultiPixelSearch(370, 7, 390, 9, 1, 1, Hex(0x576F7B, 6), $offColors1, 15)
    ...
    Local $IsMainGrayed2 = _MultiPixelSearch(370, 7, 390, 9, 1, 1, Hex(0x34434A, 6), $offColors2, 15)
    ...
    Local $IsMainGrayed3 = _MultiPixelSearch(370, 7, 390, 9, 1, 1, Hex(0x688593, 6), $offColors3, 15)
...
Func IsBuilderBaseGrayed()
    Local $offColors1[3][3] = [[0x3D5F72, 4, 0], [0x070707, 6, 0], [0x7B7B77, 8, 0]]
    Local $IsBuilderBaseGrayed1 = _MultiPixelSearch(440, 7, 465, 9, 1, 1, Hex(0x576F7B, 6), $offColors1, 15)
    ...
    Local $IsBuilderBaseGrayed2 = _MultiPixelSearch(440, 7, 465, 9, 1, 1, Hex(0x34434A, 6), $offColors2, 15)
    ...
    Local $IsBuilderBaseGrayed3 = _MultiPixelSearch(440, 7, 465, 9, 1, 1, Hex(0x688593, 6), $offColors3, 15)
